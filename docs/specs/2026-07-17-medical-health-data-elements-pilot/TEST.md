...
- Viewpoint 必须直接涉及医疗健康数据治理、授权、流通、开发利用、标准互操作或支付保险数据应用；纯 AI 编程和一般医院 IT 观点即使来自行业媒体也被拒绝。
...
- fact 与 viewpoint 使用完全相同的证据 URL 集合时只保留排序更高的一条；多证据观点与单个事实只有部分证据重叠时仍可同时保留。

## 5. 30 天历史回填与 7 天试跑报告

- 30 天历史窗口按 Signal / Event 的发布日期计算，7 天连续验证按不同日历日的 source run 计算；
- 公开来源流与试跑报告使用相同的 30 天 Signal 日期边界，窗口外累计 Signal 不进入本轮公开计数；
...
- 顶部品牌区显示“医疗健康数据要素情报站”和最近更新时间 / 内容数量，首页主体不再出现独立站点介绍标题；
- 根首页首屏展示真实的重点 Event 或 Viewpoint；存在事实 Event 时依次展示正在形成的方向、重点事件、观点与文章和最新来源动态；
- 首页观察方向卡片弱化 Track 名称，以最新 Event 标题和摘要为主要内容，并继续显示事件数、独立来源数和详情入口；
