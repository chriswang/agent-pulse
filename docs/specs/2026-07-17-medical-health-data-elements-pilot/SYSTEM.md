...
`rules.json` 经过 Zod 校验，包含强相关词、上下文词、排除词、实体别名、竞品、来源栏目映射、Track 规则、行业影响权重和发布门禁。标题命中防汛、疾病通报等明确排除项且标题本身没有数据要素强相关词时直接排除；列表摘要即使串入相邻文章的数据关键词，也不能覆盖标题排除项。
...
统一 Top 10 合并已发布 Event 与 Viewpoint，保留 `fact` / `viewpoint` 类型、证据状态和原文入口。Event 按确定性影响、价值、置信度和热度排序；Viewpoint 按可审计关注度、来源权威、时效和目标受众覆盖排序。事实与观点如果使用完全相同的一组证据 URL，只保留排序更高的对象，避免同一材料跨类型重复占位；聚合多条证据形成的观点不会因与单个事实部分重叠而被删除。没有合格对象时不凑数。
...
完整验证设置 `MODEL_ENRICHMENT_ENABLED=true`。首次回填必须有真实的 30 天日期下界，并对同源分页设置页数、响应体和速率上限；不能把“清空游标后重抓首页”称为历史回填。确定性路径负责事实摘要、主体、主线、原始证据、热度计算与最终排序。AI 失败不丢弃采集结果：workflow 仍生成报告、保存采集快照并发布确定性结果，但最后的完整验证门禁必须失败，不能用绿色 Actions 状态掩盖观点结果缺失；当日模型分析记为失败且不能计入完整 7 天通过条件。试跑报告只采用证据 URL 仍存在于当前快照的 Viewpoint；空快照或与当前快照不匹配的历史成功结果不会污染报告和 CI fixture。报告与公开来源流使用同一个 30 天证据窗口；中国 / 国际内容比例按发布机构地域计算，不因国际材料被翻译成中文而改写来源归属。Pages 使用独立 `industry-pages.yml`，从行业快照和 Viewpoint 报告构建根首页。

首次真实回填前必须清理旧基线的 Signal、Event、聚类关系与运行计数，保留来源目录和最新来源审计；验证窗口从新规则首次成功运行重新计时。连续性按七个独立日历日的完整 run 计算，不能只按最早时间推算。

## 8. 公开首页

行业首页是面向业务决策者的情报入口，不是工程运行或试跑验收看板。信息顺序固定为：

- 顶部品牌区承载站点名称、最近更新时间和事实 / 观点 / 动态数量，不再使用独立大幅站点介绍占据首页首屏；
- 首屏“今日重点变化”，使用排序最高且已通过门禁的 Event 或 Viewpoint，并显示日期、来源数和详情入口；
- 已经出现事实 Event 的“正在形成的行业方向”，卡片以最新 Event 标题和摘要为主体，Track 名称降为轻量分类上下文；未达到长期趋势门槛时明确标记为观察方向；
- 可回链原始资料的重点事实事件；只有真实传播证据时才能进一步标记为热点；
- 与事实分开展示的观点与文章，只有真实互动或多来源传播证据才能标记热门；
- 尚未形成 Event 的最新来源动态，保留弱信号发现能力。

当前试跑天数、信源成功率、采集运行次数、模型状态与 Token、来源健康、聚类准确率、人工评审和时间节省属于运行信息，移至来源页、产品方法页或机器报告，不在首页主体展示。

没有数据时显示真实空状态，不回退展示上游 AI 演示数据；有 Signal 但没有趋势时继续展示最新来源动态，不能只显示“等待有效证据”，也不能把弱信号包装成趋势或热门。
