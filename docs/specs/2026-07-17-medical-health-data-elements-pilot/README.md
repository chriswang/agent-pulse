...
- 状态：首轮信源验证通过；正在实施过去 30 天历史回填 + 未来 7 天连续验证
- 日期：2026-07-17
- 目标版本：Unreleased
- 上游基线：`barretlee/agent-pulse@b86923f`

本规格把 Agent Pulse 作为持续升级的上游产品内核，在不复制其 AI 行业数据的前提下，增加一个公开的“医疗健康数据要素”行业包。行业包拥有独立信源、领域规则、过去 30 天历史证据、7 天连续验证、趋势叙事、仓库快照、首页和 GitHub Actions；模型通过 OpenAI-compatible 适配层接入火山方舟 `glm-5.2`，但本轮明确不调用模型。
...
最新 GitHub source audit 已达到 18 个中国独立发布机构和 3 个国际发布机构。后续不再空等 30 天：首次运行一次性回填过去 30 天，随后连续运行 7 个日历日；模型保持暂停。
