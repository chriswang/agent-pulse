...
export const releases = [
  {
    ...
    changes: [
      "首页观察方向卡片弱化分类名称，突出每个方向下最新事实事件的标题与摘要",
      "医疗健康数据要素站将名称、最近更新时间和内容数量收进顶部品牌区，首页首屏直接展示今日重点情报",
      "医疗健康数据要素首页改为情报优先：首屏展示重点变化，并依次呈现观察方向、重点事件、观点文章与最新来源动态；试跑、采集、信源健康和 Token 指标移出首页",
      ...
      "公开完整性校验按 30 天历史证据窗口核对 Event 数量，历史回填事实不会再被 7 天运行起点误判为计数不一致",
      "无模型事实 Event 的标题必须包含可验证动作，相关评论、解读与观点只保留为来源线索",
      "标题排除项优先于被污染的列表摘要，防汛和疾病通报不会因相邻数据关键词误入行业结果",
