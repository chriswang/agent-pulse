...
- 修复方舟 `glm-5.2` 观点聚类在 4,000 Token 完成预算下再次被内部推理截断、但 Actions 仍显示绿色的问题：观点输出收敛为最多 3 个聚类，显式使用低推理强度和 8,000 Token 完成预算；采集快照与确定性结果仍可保存，但模型分析失败会在 workflow 末尾明确阻断完整验证。
