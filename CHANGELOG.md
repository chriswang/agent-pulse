...
- 医疗健康数据要素验证改为“过去 30 天历史证据回填 + 未来 7 个日历日连续验证”：首次运行会重置旧行业情报、按发布日期下界执行同源有界分页，并在模型完全停用时用确定性规则生成中文普通事实 Event 和六条主线关联。趋势门槛未满足前，页面也会展示有效 Signal、事实 Event、独立发布机构与原始证据链接，不再用空白页或“等待有效证据”代替已采集事实；该能力已完成本地实现与测试，真实 GitHub 回填结果仍以工作流验收为准。
