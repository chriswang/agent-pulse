...
- 行业模式新增确定性相关性与排除规则、中文双字聚类、发布机构级独立证据、约 80/20 的中国/国际公开配额、中文模型输出校验和 30 天基线 Narrative；旧的 264 条英文混杂 Signal 与 30 个单来源 Event（其中 7 个已发布、6 个落在原试跑窗口）已从新基线快照移除。
- 医疗健康六条趋势补齐独立的来源主题映射，并按中国来源优先、健康状态优先展示；基线期即呈现各主线持续观察的政策、数据基础设施、医保商保、药械、竞品和标准会议来源，但仍只把通过多来源证据门禁的内容计为 Event 与趋势证据。
